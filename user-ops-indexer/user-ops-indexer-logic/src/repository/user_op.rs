...
pub async fn find_user_op_by_op_hash(
    ...
    let res = db
        .query_one(
            Entity::find_by_id(op_hash.as_slice())
...
pub async fn list_user_ops(
    ...
    if let Some(sender) = sender_filter {
        q = q.filter(Column::Sender.eq(sender.as_slice()));
    }
    if let Some(bundler) = bundler_filter {
        q = q.filter(Column::Bundler.eq(bundler.as_slice()));
    }
    if let Some(paymaster) = paymaster_filter {
        q = q.filter(Column::Paymaster.eq(paymaster.as_slice()));
    }
    if let Some(factory) = factory_filter {
        q = q.filter(Column::Factory.eq(factory.as_slice()));
    }
    if let Some(tx_hash) = tx_hash_filter {
        q = q.filter(Column::TransactionHash.eq(tx_hash.as_slice()));
    ...
    if let Some(entry_point) = entry_point_filter {
        q = q.filter(Column::EntryPoint.eq(entry_point.as_slice()));
    }
...
pub async fn stream_unprocessed_logs_tx_hashes(
    ...
    from_block: u64,
    to_block: u64,
) -> Result<impl Stream<Item = B256> + '_, anyhow::Error> {
...
WHERE logs.address_hash    = $1
  ...
  AND user_operations.hash IS NULL"#,
    ...
    .filter_map(|tx| async {
        match tx {
            Ok(tx) => Some(B256::from_slice(&tx.transaction_hash)),
            Err(err) => {
...
mod tests {
    ...
    async fn find_user_op_by_op_hash_ok() {
        ...
        let hash = B256::from(U256::from(0x0102));
        let item = find_user_op_by_op_hash(&db, hash).await.unwrap();
        assert_eq!(item, None);

        let hash = B256::from(U256::from(0x0101));
        let item = find_user_op_by_op_hash(&db, hash).await.unwrap();
        assert_ne!(item, None);
        ...
        let hash = B256::from(U256::from(0x1a0401));
        let item = find_user_op_by_op_hash(&db, hash).await.unwrap();
        assert_ne!(item, None);
        let item = item.unwrap();
        assert_eq!(item.hash, hash);
        ...
        let hash = B256::from(U256::from(0x1a0e01));
        let item = find_user_op_by_op_hash(&db, hash).await.unwrap();
        assert_ne!(item, None);
        let item = item.unwrap();
        assert_eq!(item.hash, hash);
    ...
    async fn list_user_ops_ok() {
        ...
        let (items, next_page_token) = list_user_ops(
            ...
            None,
            None,
            Some(B256::from(U256::from(0x0504))),
            None,
            Some(0),
            Some(0),
            None,
    ...
    async fn find_unprocessed_logs_tx_hashes_ok() {
        ...
        let items: Vec<B256> = stream_unprocessed_logs_tx_hashes(&db, entrypoint, topic, 100, 150)
            ...
            .collect()
            .await;
        assert_eq!(items, [B256::from(U256::from(0xffff))]);
    }
}
